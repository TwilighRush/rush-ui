...
`Input` 是 `@rush-ui/react` 的基础单行文本输入组件，用于后台系统中的表单、筛选器、表格工具栏和搜索场景。它基于原生 `<input>` 渲染，保留浏览器默认输入行为，同时统一尺寸、错误态、只读态、禁用态、清空按钮、字数统计、图标插槽、前后缀和焦点样式。
...
- 首版不提供多行文本输入，后续应由 `Textarea` 独立设计。
