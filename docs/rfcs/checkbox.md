...
`Checkbox` / `CheckboxGroup` 是 `@rush-ui/react` 的基础复选组件，用于后台系统中的确认项、多选筛选、批量选择和权限配置。它基于原生 `<input type="checkbox">` 渲染，保留浏览器默认勾选、键盘和表单行为，同时统一尺寸、半选、说明文本、组级错误态、禁用态和焦点样式。
